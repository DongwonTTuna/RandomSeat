...
export function GridCell(props) {
  ...
  const InputOnChange = (e) => {
    items[items.findIndex((item) => item.id === parseInt(e.target.closest('.mode').id))].name = e.target.value
    dta.items = items
  ...
  const Dbcl = (e) => {
    if (e.currentTarget.classList.contains("student")) {
      ...
      items[items.findIndex((item) => item.id === parseInt(e.currentTarget.id))].mode = 2
      ...
      items[items.findIndex((item) => item.id === parseInt(e.currentTarget.id))].mode = 1
    } else {
      ...
      items[items.findIndex((item) => item.id === parseInt(e.currentTarget.id))].mode = 0 
    }
    setItems(items)
    localStorage.setItem('data', JSON.stringify(dta))
  };
...
export function OnlyCell(props) {
  ...
  cparent += document
    ...
    ? " hue-rotate-30 student-pstyle hover:shadow-none"
    : " priority-pstyle ";
  cchild += document
    .getElementById(props.val)
    .classList.contains("student")
